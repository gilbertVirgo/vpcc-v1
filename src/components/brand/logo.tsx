...
/*
 * The VPCC mark: a filled disc with the wordmark knocked out of it.
 *
 * Carried over from the previous site, but painted with `currentColor` instead
 * of the hard-coded brand orange the original SVG shipped with — so it can sit
 * in accent orange on light backgrounds and in paper on the dark footer
...
export function Logo({
	className,
	title = "Victoria Park Community Church",
}: {
	className?: string;
	/** Omit only when an adjacent element already names the link. */
	title?: string;
}) {
	return (
		<svg
