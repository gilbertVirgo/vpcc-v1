...
import { eventHref, getLiveEvents } from "@/lib/events";
 ...
 * whether they appear on What's On at all — a sitemap advertising a URL that
 * 404s is worse than one that lists nothing.
 *
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
	...
	/* An event is news with a closing date on it, so it is crawled more often
	   than a standing page and ranked above one. What's On itself is already in
	   pageEntries — it is a page document like any other. */
